...
Write-Host "Shut down all instances of Visual Studio running on your computer before you begin"
Write-Host "You have 5 seconds...this will self-destruct....just kidding."
...
###Get content from separate json file that stores url's and pull from that json object

$url = Get-Content '' -raw | ConvertFrom-Json

###Functions to start services
function Start-GatewayApiService{
  $gatewayUrl= $url.GatewayApi.Path
  Start-Process '' -ArgumentList "/path:$gatewayUrl" 
}
